...
import de.johannesluderschmidt.throng.manualGUI.model.ThrongClient;
import de.johannesluderschmidt.throng.manualGUI.model.ThrongLoader;
			...
			try{
				try{
					if(oscOutputPort!= null){
						...
						ThrongProxy.getInstance().setInboundPorts(oscPortsArray);
						InetAddress outboundIp = InetAddress.getByName(outputIp.getText());
						ThrongProxy.getInstance().setOutboundConnection(outboundIp, Integer.parseInt(oscOutputPort.getText()));
						ThrongProxy.getInstance().startProxy();
					}
					startStopButton.setText("Stop Throng");
					floscStarted = true;
				}catch(UnknownHostException exc){
					Debug.writeException("Could notresolve host: "+outputIp.getText()+". Message: "+exc.getMessage(),this, exc);
				}catch(Exception ex){
					Debug.writeException("Error when trying to start Flosc: "+ex.getMessage(),this, ex);
				}
