...
public class OscMessage {
    ...
    */
	...
	public Vector<Character> getTypes() {
		return types;
	}

	public void setTypes(Vector<Character> types) {
		this.types = types;
	}
	...
	public String toString(){
		...
		oscMessageString = oscMessageString + "name: " + name;
		
		//types
		oscMessageString = oscMessageString + ", [types: ";
		int first = 0;
		for(Character c:types){
			if(first != 0){
				oscMessageString = oscMessageString + ", " + c;
			}else{
				oscMessageString = oscMessageString + c;
			}
			first = first+1;
		}
		oscMessageString = oscMessageString + "]";
		
		//arguments
		oscMessageString = oscMessageString + ", [arguments: ";
		first = 0;
		for(Object o:arguments){
			if(first != 0){
				oscMessageString = oscMessageString + ", " + o;
			}else{
				oscMessageString = oscMessageString + o;
			}
			first = first+1;
