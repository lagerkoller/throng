		...
		if (address.equals("/tuio/2Dobj")) {
			...
			if (command.equals("source")) {
				...
				if (objectList.get(s_id) == null) {
					...
					TuioObject addObject = new TuioObject(s_id,c_id,xpos,ypos,angle,this.src, currentFrame);
					...
					if ((tobj.xpos!=xpos) || (tobj.ypos!=ypos) || (tobj.angle!=angle) || (tobj.x_speed!=xspeed) || (tobj.y_speed!=yspeed) || (tobj.rotation_speed!=rspeed) || (tobj.motion_accel!=maccel) || (tobj.rotation_accel!=raccel)) {
						...
						TuioObject updateObject = new TuioObject(s_id,c_id,xpos,ypos,angle,this.src, currentFrame);
					...
					while(frameEnum.hasMoreElements()) {
						...
						switch (tobj.getTuioState()) {
							...
							case TuioObject.TUIO_ADDED:
								TuioObject addObject = new TuioObject(currentTime,tobj.getSessionID(),tobj.getSymbolID(),tobj.getX(),tobj.getY(),tobj.getAngle(), this.src, fseq);
							...
							default:
								...
								if ( (tobj.getX()!=updateObject.getX() && tobj.getXSpeed()==0) || (tobj.getY()!=updateObject.getY() && tobj.getYSpeed()==0) )
									updateObject.update(currentTime,tobj.getX(),tobj.getY(),tobj.getAngle(), fseq);
								else
									updateObject.update(currentTime,tobj.getX(),tobj.getY(),tobj.getAngle(),tobj.getXSpeed(),tobj.getYSpeed(),tobj.getRotationSpeed(),tobj.getMotionAccel(),tobj.getRotationAccel(),fseq);
			...
			if (command.equals("source")) {
				...
				if (cursorList.get(s_id) == null) {
					...
					TuioCursor addCursor = new TuioCursor(s_id, -1 ,xpos,ypos, this.src, currentFrame);
					...
					if ((tcur.xpos!=xpos) || (tcur.ypos!=ypos) || (tcur.x_speed!=xspeed) || (tcur.y_speed!=yspeed) || (tcur.motion_accel!=maccel)) {
						...
						TuioCursor updateCursor = new TuioCursor(s_id,tcur.getCursorID(),xpos,ypos, this.src, currentFrame);
					...
					while(frameEnum.hasMoreElements()) {
						...
						switch (tcur.getTuioState()) {
							...
							case TuioCursor.TUIO_ADDED:
								...
								TuioCursor addCursor = new TuioCursor(currentTime,tcur.getSessionID(),c_id,tcur.getX(),tcur.getY(), this.src, currentFrame);
							...
							default:
								...
								if(this.src == null || !this.src.equals(updateCursor.getSrc())){
									updateCursor.setSrc(this.src);
								}
								if ( (tcur.getX()!=updateCursor.getX() && tcur.getXSpeed()==0) || (tcur.getY()!=updateCursor.getY() && tcur.getYSpeed()==0) )
									...
									updateCursor.update(currentTime,tcur.getX(),tcur.getY(),tcur.getXSpeed(),tcur.getYSpeed(),tcur.getMotionAccel(), fseq);
